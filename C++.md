...
- **static**
  ...
  - static对于变量
    ...
    2. 全局变量
       ...
       作用域：全局静态变量在声明他的文件之外是不可见的。准确地讲从定义之处开始到文件结尾。(只能在本文件中存在和使用)【全局变量在a.cpp里声明定义此变量，在b.cpp里使用此变量 extern var;】
  ...
  - static对于函数
       ...
       修饰普通函数，表明函数的作用范围，仅在定义该函数的文件内才能使用。在多人开发项目时，为了防止与他人命名空间里的函数重名，可以将函数定位为 static。（和全局变量一样限制了作用域而已）【普通的可以被外部链接，static普通将作用域局限于当前cpp文件】
  ...
  - static对于类
    ...
    1. 成员变量
       ...
       要么类外初始化、要么类内const修饰static类内初始化。
...
## static初始化时机和线程安全问题
所有在全局区域的变量（静态局部变量/全局变量/全局类对象）都在编译阶段就分配好了内存并初始化。可以理解为静态初始化。

而动态初始化（运行时初始化）主要指需要经过函数调用才能完成的初始化（比如在函数内部做静态变量，静态类的初始化）
...
具体实现如下：**局部静态变量（变相理解在函数里的static）**在编译时，编译器的实现是和全局变量类似的，均存储在bss段中。然后编译器会生成一个`guard_for_bar` 用来保证线程安全和一次性初始化的整型变量，是编译器生成的，存储在 bss 段。它的最低的一个字节被用作相应静态变量是否已被初始化的标志， 若为 0 表示还未被初始化，否则表示已被初始化(` if ((guard_for_bar & 0xff) == 0)`判断)。 `__cxa_guard_acquire` 实际上是一个加锁的过程， 相应的 `__cxa_guard_abort` 和` __cxa_guard_release` 释放锁。
...
可以看到引用和常量指针的汇编代码是一模一样的，所以可以得出在底层，引用就是一个**常量指针**。
...
//const修饰的是整个pint pint是指针 则是常量指针
...
C++出来后，C++11之前，左值遵循了C语言的分类法，但与C不同的是，其将非左值表达式统称为右值，函数为左值，因为c++有引用这个东西 。并添加了**引用能绑定到左值但唯有const的引用能绑定到右值的规则**。
...
7. 由指针访问成员（ `->` ）运算符的结果
8. 下标运算符的结果(`[]`)
9. 字符串字面值("abc")：字符串字面值在编译时被分配在静态存储区（通常是只读数据段），程序整个运行期间都存在，有明确的地址。`&"hello"` 是合法的。字符串字面值是数组类型，数组是左值。历史原因早期字符串字面值是可修改的 char []，C++为了强化类型安全，将字面值改为const char[],但依然保留了左值身份可以取地址，可以绑定到const char*上。
...
2. 返回值为非引用的函数调用或操作符重载，例如：str.substr(1, 2), str1 + str2, or it++（临时变量）
3. 后置自增和自减表达式(a++, a--)
4. 算术表达式（x + y;）
5. 逻辑表达式
6. 比较表达式
...
T(T& a){
	m_val = new xx(a.m_val); 
}

//会造成指针悬挂
//实参b作为右值引用传入形参a，b和T的m_val同时指向x。构造结束b销毁m_val指向空，T的m_val指向为空
T(T&& a){
	m_val = a.m_val; 
}

//正确代码
//实参b传入后，T指向x，b指向x。构造结束b销毁不影响T
T(T&& a){
	m_val = a.m_val; 
	a.m_val=nullptr; 
}
```
（不正确的拷贝构造）
从类的移动构造开始说。由于一个带有堆内存的类必须提供一个深拷贝拷贝构造函数，因为默认的拷贝构造函数是浅拷贝，会发生“指针悬挂”的问题。（如case1，实参被销毁会导致指针悬挂，正确案例case2）

（引入移动拷贝构造）
提供深拷贝的拷贝构造函数虽然可以保证正确，但是在有些时候会造成额外的性能损耗，因为有时候这种深拷贝是不必要的。

（默认的移动拷贝构造：只会移动，不会置空case3）
如果不提供深拷贝的拷贝构造函数，上面的测试代码将会发生错误。内部的m_ptr将会被删除两次，一次是临时右值析构的时候删除一次，第二次外面构造的T对象释放时删除一次，而这两个对象的m_ptr是同一个指针，这就是所谓的指针悬挂问题。

（正确的移动拷贝构造：case4）
这个构造函数并没有做深拷贝，仅仅是将指针的所有者转移到了另外一个对象，同时，将参数对象a的指针置为空，这里仅仅是做了浅拷贝，因此，这个构造函数避免了临时变量的深拷贝问题。

**移动的意义：减少内存拷贝开销。实现时需注意，有内存的成员移动后需将源值置空。**

**进而引出了移动语义的概念(std::move)。**

当编译器看到&&的时候会判定为右值引用，对编译器来说这是一个临时变量的标识，对于临时变量来说我们仅仅做一次浅拷贝就行，不需要深拷贝，从而解决了前面提到的临时变量拷贝构造产生的性能损失的问题。【改变指向，不要拷贝】这就是所谓的移动语义，右值引用的一个重要作用是用来支持移动语义的。那我们知道了移动语义是通过右值来匹配临时值的，那么很自然会想到，普通的左值是否也能借助移动语义来优化性能呢？C++11为了解决这个问题，提供了std::move方法来将左值转换为右值，从而方便应用移动语义。**move是将对象资源的所有权从一个对象转移到另一个对象，只是转移，没有内存的拷贝，这就是所谓的move语义。**
...
```
传入右值时，`forwardValue(T& val)`只接收左值，所以右值被匹配到`forwardValue(const T& val) `，右值类型变成了`const左值引用`

会造成什么问题
> 如果 processValue 有一个移动版本（如 processValue(MyType&&)），由于 val 是左值，这个版本永远不会被调用。

> 如果 processValue 接受非 const 左值引用（如 processValue(MyType&)），那么第二个重载中的 const T& 无法绑定到 MyType& → 编译错误。

> 如果 processValue 按值传递，则右值本应触发移动构造，但现在因为 val 是 const 左值，会触发拷贝构造（若类型只支持移动，则编译失败）。

**引入完美转发的概念。**

C++11引入了完美转发解决这个问题：在函数模板中，完全依照模板的参数的类型（即保持参数的左值、右值特征），将参数传递给函数模板中调用的另外一个函数。C++11中的std::forward正是做这个事情的，他会按照参数的实际类型进行转发。

**有指针的成员变量需定义拷贝构造函数(拷贝/移动)，否则会出现指针悬挂**
...
用于控制变量的作用域、生命周期以及存储位置

（1）auto存储类（自动存储类，即局部变量，在进入作用域时创建，离开时销毁）

（2）static存储类 （静态存储类，作用域限制）
> 局部变量：程序内只初始化一次，生命周期同程序
> 全局变量：文件内有效
> 类内：变量所有类对象共享，必须在类外定义和初始化。函数不包含this指针，只能访问静态成员变量

（3）extern存储类  (外部存储类)

（4）register存储类（寄存器存储类）
> 编译器将变量存储在 CPU 寄存器中，以提高访问速度
> 主要用于频繁使用的局部变量（如循环计数器）。 后废弃但合法
...
```
T foo(this*, var...) { //相当于函数的第一个参数是this指针。和go很像吧
    this->m;
}
```
...
**this 指针是编译器在编译阶段自动生成的。在成员函数调用时被传入成员函数。类的地址**
...
### 如果在类的析构函数中调用delete this，会发生什么？OOM
...
3. 在类声明中定义的函数 `.h文件` ，除了虚函数的其他函数都会自动隐式地当成内联函数。
...
- 优点
  - 内联函数同宏函数一样将在被调用处进行代码展开，**省去了参数压栈、栈帧开辟与回收，结果返回等，从而提高程序运行速度。【减少函数调用的开销】**
  - 内联函数相比宏函数来说，在代码展开时，会做**安全检查或自动类型转换**（同普通函数），而宏定义则不会。
  - 内联函数在**运行时可调试**，而宏定义不可以。
  - 可以说inline函数不仅吸收了了C宏定义的，同时消除宏定义的缺点。
- 缺点
  - 代码膨胀。内联是以代码膨胀（复制）为代价，消除函数调用带来的开销。**增加代码量**
...
- **在c语言中**
  ...
  对于代码1。**test3**是结构体标签，而**test4**相当于变量类型，即`struct test3 = test4` **【类型别名】** 定义变量的时候 `struct test3 t; test4 t;`两者等价

  对于代码2。类型是struct test3，变量是test4 **【定义变量】**

- **在c++中**

  对于代码1。**类型别名**

  对于代码2 。这是c++的写法，test3是类型，test4是变量。



## explicit 关键字

explicit关键字主要是用来修饰类中的构造函数的，对于仅有一个参数或除第一个参数外其余参数均有默认值的类构造函数（形参默认值），尽可能使用explicit关键字修饰。因为只有一个参数或者除了第一个参数其他参数是默认参数的构造函数来说，他还有另一个名字叫做**转换构造函数**。
...
友元的作用是提高了程序的运行效率（即减少了类型检查和安全性检查等都需要时间开销），但它**破坏了类的封装性**和隐藏性，使得非成员函数可以访问类的私有成员。

> 我碰到了一种必须使用友元的情况。如下图【这个例子不好，struct成员变量默认是public访问权限，应改成class】
>
...
1、每个类都有自己的虚函数表【函数指针数组】，虚函数表里有每个虚函数地址。如果没有重写则用父类的虚函数对应地址，如果重写了用自己的虚函数地址。自己本身的虚函数放在父类虚函数后面。**既不存在于堆，也不存在于栈**【非虚函数是静态绑定的，在编译时直接确定调用地址，不会出现在虚函数表中】

2、如果存在多重继承，则会将每个父类的虚函数表组合起来成为虚函数表指针。**跟着对象进行分配的，对象在堆他就在堆上，在栈就在栈上**

3、找的时候先根据虚函数表指针vptr找到虚函数表vtable，再拿出对应虚函数的地址。
...
这样就会有内存和执行速度方面的成本：【内存和调用的开销】

1. 每个对象都会增大，因为在对象最前面的位置加入了指针（虚函数表指针）
2. 对于每个类，编译器都会创建虚函数地址表（重写比较少会有地址空间浪费，多重继承vptr会4N方式增加）
3. 对于每个函数调用，都要查找表中地址
...
- **构造函数可以设置为虚的吗？**
  ...
  答：不能。因为虚函数的调用是需要通过“虚函数表”来进行的，而虚函数表也需要在对象实例化之后才能够进行调用。在构造对象的过程中，还没有为“虚函数表”分配内存。所以，这个调用也是违背先实例化后调用的准则。【实例化结束后才存在虚函数表】

  子类的默认构造函数总要执行的操作：先调用父类构造函数
...
- **c++虚析构函数**（父类必须声明虚析构函数）
  ...
  如果析构函数不是虚的，那么编译器只会调用对应指针类型的虚构函数。编译器根据 pe 的静态类型 Employee* 调用 Employee::~Employee(), Singer的析构函数不会调用。

  如果析构函数是虚的，根据虚函数机制动态找到Singer，调用Singer的虚构函数，再自动调用Employee的虚构函数。

  **本质是为了确保积累指针删除派生类对象时，能正确调用派生类的析构函数**
...
- **哪些函数不能是虚函数？**
  ...
  2.  静态与非静态成员函数之间有一个主要的区别。那就是静态成员函数没有隐藏的this指针。对于**虚函数，它的调用恰恰需要this指针**。在有虚函数的类实例中，this指针调用vptr指针，vptr找到vtable(虚函数列表)，通过虚函数列表找到需要调用的虚函数的地址。总体来说虚函数的调用关系是：this指针->vptr->vtable ->virtual虚函数。所以说，static静态函数没有this指针，也就无法找到虚函数了

  ③**内联函数**也不能是虚的，因为要在编译的时候展开，而虚函数要求动态绑定。另外就是虚函数的类对象必须包含vptr，但是内联函数是没有地址的，编译的时候直接展开了所以不行。

  ④**构造函数**也不行。

  ⑤**成员函数模板不能是虚函数**。因为c++ 编译器在解析一个类的时候就要确定虚函数表的大小，如果允许一个虚函数是模板函数，那么compiler就需要在parse这个类之前扫描所有的代码，找出这个模板成员函数的调用（实例化），然后才能确定vtable的大小，而显然这是不可行的，除非改变当前compiler的工作机制。因为类模板中的成员函数在调用的时候才会创建(创建一个类虚函数表大小就是确定的，但类模板成员函数是在调用的时候才会被创建两者相悖)

- **虚函数表是共享还是独有的？**

  答：**虚函数表是针对类的，一个类的所有对象的虚函数表都一样。** 在gcc编译器的实现中虚函数表vtable **存放在可执行文件的只读数据段.rodata中**。是编译器在编译器为我们处理好的。
...
- **构造函数或析构函数中调用虚函数会怎样？**
  ...
  首先不应该在构造函数和析构函数中调用虚函数。（在其中调用虚函数，会执行基类的虚函数，达不到动态绑定的效果。构造函数里子类还没创建出来，析构函数里子类先析构）
...
虚表地址：一个对象的地址的前四个字节。（指向另一块地址了）
虚表地址：把虚表地址转换成int*（数组指针），一个int *表示一个数组元素。解引用就是虚函数地址。下一个就是以外层int *为单位+1
...
- 只能在栈上生成对象的类。（operator new/delete私有，构造函数共有）
...
public:
    static Logger& getInstance() {
        static Logger instance;  // 懒汉单例（C++11 起线程安全）
        return instance;
    }
};
...
private:
    ...
    ~RefCounted() {}
    int refs = 0;
    friend class Handle;  // 只有 Handle 可以创建/销毁
};
```
...
**拷贝构造函数：**拷⻉构造函数的函数参数为**对象本身的引用**，用于根据⼀个已存在的对象复制出⼀个新的该类的对象，⼀般在函数中会将已存在的对象的数据成员的值⼀⼀复制到新创建的对象中。如果没有显示的写拷⻉构造函数，则系统会默认创建⼀个拷⻉构造函数，但当类中有指针成员时，最好不要使⽤编译器提供的默认的拷⻉构造函 数，最好⾃⼰定义并且在函数中执⾏深拷⻉。

**移动构造函数：**【**变量移动后置空**】有时候我们会遇到这样一种情况，我们用对象a初始化对象b后对象a我们就不在使用了，但是对象a的空间还在呀（在析构之前），既然拷贝构造函数，实际上就是把a对象的内容复制一份到b中，那么为什么我们不能直接使用a的空间呢？这样就避免了新的空间的分配，大大降低了构造的成本。这就是移动构造函数设计的初衷。拷贝构造函数中，对于指针，我们一定要采用深层复制，而移动构造函数中，对于指针，我们采用浅层复制。
...
> 对象不会被创建，析构函数不会被调用
> 申请的内存会造成内存泄漏，使用try-catch或RAII管理（用智能指针管理需要分配内存的变量），不要吞异常
...
析构函数不能抛出异常原因有两个：（资源未及时释放、多个析构函数异常栈展开导致程序崩溃）
...
>先基类构造函数，成员变量构造函数，再子类构造函数
>析构函数相反
>对象创建顺序：先分配静态变量，再分配成员变量、最后调用构造函数设置初始值
...
> 按照类中声明顺序给默认值，初始化列表不影响成员变量初始化顺序（遵循声明顺序）
> 构造函数中初始化，初始化按照代码顺序
> static在类外初始化，const在初始化列表初始化
...
**1、构造函数**（名字不一样，基类的private成员需要通过基类的构造函数初始化，派生类不能直接访问）
...
**3、赋值运算符=**(主要是语义吧，继承不能完成=的语义)

>编译器总是为派生类生成默认版本，隐藏了基类的版本。
>参数列表不同（const Base& vs const Derived&），导致基类版本无法直接用于派生类对象。
>继承赋值运算符可能导致部分赋值（只复制基类，不复制派生类成员），造成对象状态不一致。
>C++ 设计哲学强调：派生类需要显式决定如何复制自身，包括基类部分，不能隐式从基类获取赋值行为。
...
如果不是虚继承，那么CChild直接调用Cparent的构造函数，这是成立的，而且CChild是不需要调用CFinalClassMixin的构造函数。若把它声明为虚继承（派生类对象一定会调用基类的构造函数），那么CChild就必须负责CFinalClassMixin构造函数的调用，这样又因为不能继承friend类，所以不能调用，造成了错误。（看不懂 不了解）
...
> malloc和free不能调用构造函数和析构函数（库函数不在编译器的控制权限内）
...
- **new[]和delete[]**
  ...
  如果new和delete[]使用，delete的时候会找前4个字节，看要释放的内存有多大。但是使用new导致没有设备区部分4个字节用来存放数组长度，这样会导致这4个字节是未定义的，因此会调用不定次的delete。同时比如说其实地址为A，应该从A开始释放，现在会从A-4开始释放。（一方面delete会去头部找数组大小，实际new是不存放的，导致delete的次数不定。另一方面new不存放会导致delete的地址偏移有问题）

  如果new[]和delete一起使用，程序会认为这是一个对象占用的空间，而不是数组，因此就析构一次。同时释放的是new[]中表示长度的前4个字节的地址，应该从A-4开始释放，如果不从头释放的话回出问题。（和上面一样，次数和偏移）



## free释放内存的理解

> 其实malloc算法有很多，除了glibc使用的ptmalloc以外，还有tcmalloc, jemalloc等等。但总体上个人认为主要分两个流派。第一个流派是ptmalloc为代表的隐藏头风格，第二个流派就是以tcmalloc和jemalloc为代表的位图风格。且就目前的情况看，后者在性能上要占据上风。其中一个重要原因在于，隐藏头风格的算法，元数据（即chunk size）之间间隔太远，不利于CPU cache命中。（我之前了解的是直接malloc，和gcc里面的链表）
...
不使用智能指针会造成的问题：容易忘记释放内存（析构里忘记释放内存）、重复释放（没提供拷贝构造拷贝赋值函数） **所以需要手动写析构、拷贝构造、拷贝赋值**

智能指针的实现

1、不允许隐式转换，提供一个构造函数
2、析构函数释放内存
3、不提供拷贝构造/赋值，提供移动构造/赋值
4、提供重载* 重载-> get release(释放原始指针管理能力) reset(重置为p) swap函数

共享智能指针的实现
1、构造函数初始化计数器（计数器指针，要被多个类共享）
2、析构函数减少计数器，计数器为空则delete，否则只将本类的ptr和count置空，不影响其他类
3、拷贝构造/赋值 增加被拷贝者计数器
4、移动构造/赋值 变量转移后置空
5、提供重载* 重载-> get release(释放当前对象)  reset(重置)
...
void riskyFunction() {
    ...
    // 危险：没有拷贝构造函数，浅拷贝会导致 double free
    IntArray arr2 = arr;       // 编译通过，运行崩溃！
}
...
#### auto_ptr（即不独享，也不共享，所以仍有普通内存管理拷贝的问题）

最早的智能指针，c++11之后就删除了，有以下问题：

1. auto_ptr不能指向一组对象，不能和new[]一起使用
2. auto_ptr不能和标准容器一起用
3. 容易野指针。有两个auto_ptr比如说`p1和p2`，当在函数参数间传递指针的时候，所有权也会发生转移。具体来说比如p1指向一块内存，然后p2是p1的拷贝，因此p2也指向了这块内存。当函数调用完后，p2指向的这块内存释放掉了，p1不就成为了一个野指针。
...
> 计数器是线程安全的。但智能指针管理的对象的数据可能是不安全的
多个线程同时修改不同的 shared_ptr（安全）
多个线程同时修改同一个 shared_ptr（不安全）（需要外部锁）一方面是shared_ptr本身是线程不安全的，它的reset 或者构造函数等函数内部操作不是原子的即ptr的操作。另一方面shared_ptr管理的对象操作也是不安全的，即ptr的内部数据的操作。
多个线程同时读取同一个 shared_ptr（安全）
>
> 对于shared_ptr内部ptr的线程不安全就是加锁使用临时变量的方法解决，保证最少计数器保证ptr存在。
> 对于ptr数据的线程安全：给操作这个ptr数据的函数加锁

```c++
// 初始状态
global_sp 指向对象 O，引用计数 = 1
...
// 线程A执行 safe_use()
{
    ...
    local = global_sp;  
    // 拷贝操作：
    // 1. local.ptr_ = global_sp.ptr_
    // 2. local.cb_ = global_sp.cb_
    // 3. 原子操作：引用计数从 1 增加到 2
}
// 锁释放
...
global_sp.reset();  
// reset 操作：
// 1. 原子操作：引用计数从 2 减少到 1
// 2. 因为计数变成 1（不是0），所以不删除对象

// 线程A继续：
local->doSomething();  // 安全！对象还在，引用计数为1
// local 析构时，引用计数从1减到0，删除对象
```
...
2. make_shared方法
   ...
   > make_shared执行一次堆分配 `make_shared(T)`，而shared_ptr构造函数执行两次`shared_ptr(new T())`
   ...
   **其次使用make_shared还是异常安全的**
   `std::shared_ptr<Widget>(new Widget)`这样的写法在函数调用里可能内存泄漏，可能构造完这个参数，再构造其他参数时发生异常，第一步申请的内存会内存泄漏。
...
shared_ptr<A> pa = make_shared<A>(); //A引用计数器1
shared_ptr<B> pb = make_shared<B>(); //B引用计数器1
pa->b = pb; //B引用计数器2
pb->a = pa; //A引用计数器1
```
...
> 捕获列表：不捕获变量，值捕获，引用捕获，值捕获+引用捕获，this指针捕获
> 没有参数可以不写参数列表的
> 选项：mutable可以修改值传递的变量、exception指定函数抛出异常
> -> 显式指定返回值。可以省略 
>
...
**nullptr是C++11引用的空指针类型，NULL在C++可能因为默认是0导致二义性**
- 首先给出NULL在C和C++中的定义
...
### 成员列表初始化
> const，引用，类成员变量没有默认构造函数时必须初始化
> 初始化 > 列表初始化 > 函数体
> 列表初始化比函数体赋值少一次构造函数调用
...
void func(Args... args) {   // args 是函数参数包
    ...
    cout << "类型个数: " << sizeof...(Args) << endl;
}

// 使用示例
func();                    // 0 个参数
func(1);                   // 1 个参数，Args = {int}
func(1, 2.5, "hello");     // 3 个参数，Args = {int, double, const char*}
```
...
auto p2 = make_unique<std::string>(5, 'a');  // "aaaaa"
auto p3 = make_unique<MyClass>("param1", 100);
...
private:
    First value_;
public:
    MyTuple(First first, Rest... rest) 
        : MyTuple<Rest...>(rest...), value_(first) {}
    
    First& get() { return value_; }
    const First& get() const { return value_; }
};

template<int N, typename First, typename... Rest>
struct TupleElement {
    static auto& get(MyTuple<First, Rest...>& t) {
        return TupleElement<N-1, Rest...>::get(t);
    }
};

template<typename First, typename... Rest>
struct TupleElement<0, First, Rest...> {
    static First& get(MyTuple<First, Rest...>& t) {
        return t.get();
    }
};
...
template<typename First, typename... Rest>
void print_tuple(const MyTuple<First, Rest...>& t) {
    cout << get<0>(t);
...
std::function【std::function对C++中各种可调用实体(普通函数、Lambda表达式、函数指针、以及其它函数对象等)的封装，形成一个新的可调用的std::function对象，简化调用。比如都是函数、lambda都是加法】
std::bind【给可调用实体绑定参数返回一个可调用实体，但是可调用实体没有类型的，所以std::functio就是把这些可调用实体整个统一的参数。里面有占位符std::placeholders::_n】
```c++
//表示绑定函数 fun_1 的第三个参数为 3，而fun_1 的第一，二个参数分别由调用 f3 的第二，一个参数指定
...
>
> **重载了函数调用符()的类：**C++的类厉害的地方之一就是可以重载函数运算，就是retType operator( )(parameter...){ }的形式，这种重载了函数调用符的类可以直接让我们用类名来实现函数的功能，比如：
>
> ```c++
...
>
> 因为有多个指针指向了同一个内存，如果提前释放了这个内存，那其他指向这个地址的指针则会成为野指针，就是指针悬挂现象。同时如果所有的指针都不指向这个地址，这块地址还没被释放，就会被造成内存泄漏。所以采用计数的方式来判断这个地址上还有多少个指针，当最后一个指针不再指向这块内存的时候，就释放掉这块内存。【指针悬挂、内存泄漏】
>
> **实现引用计数的目的**
>
...
shared_ptr<int> s_ptr(p);
shared_ptr<int> s_ptr2(p); 
...
那么哈希表就是利用了数组的这种特性，即**hash表的物理存储其实是数组**。（数组+链表）
...
> **sizeof的定义**
>
> sizeof是C语言的一种单目操作符，它并不是函数。**sizeof操作符以字节形式给出了其操作数所占存储空间的大小。**操作数可以是一个表达式或括在括号内的类型名。操作数所占存储空间的大小由操作数的类型决定。作用就是返回一个对象或者类型所占的内存字节数。sizeof在头文件中typedef为unsigned int，其值在编译时即计算好了，参数可以是数组、指针、类型、对象、函数等。它的功能是：获得保证能容纳实现所建立的最大对象的字节大小。由于在编译时计算，因此**sizeof不能用来返回动态分配的内存空间的大小**。
>
> 数组——编译时分配的数组空间大小；
>
...
问：定义一个不包含任何成员变量和成员函数的空的类，对该类型sizeof，得到的结果是多少？【1】
...
问：如果在空类里面添加一个构造函数和析构函数，sizeof是多少？【1】

答：还是一样的。因为调用构造函数和析构函数或者其他函数只需要知道函数的地址就行，函数的地址和类的实例无关。

问：如果吧析构函数或者其他函数编程虚函数呢？【虚函数表指针4/8】
...
// 告诉 C++：有个 C 函数叫 hello
extern "C" {
    void hello();   // 去找到 "hello"，不是 "_Z5hellov"
}
int main() {
    hello();   // 现在能找到了！
    return 0;
...
void*的含义就是一个描述某块内存的指针。如果再有类型，就可以使用它。
...
源程序（source code）→预处理器（preprocessor [.i]）→编译器（compiler [.s]）→汇编程序（assembler)目标程序（object code）[.o]→连接器（链接器，Linker)可执行程序（executables）
...
> 静态编译：不依赖于其他库。但：文件大，编译速度慢，静态库发生变化程序要重新编译。
> 动态编译：文件小，编译速度快，库发生变化程序不需要重新编译。但：执行时需要依赖动态库
> 
...
```
ESP/RSP	栈指针寄存器，指向当前栈顶（最低地址）
EBP/RBP	栈帧基址，指向当前函数的栈帧底部（较高地址）
...
2. 类模板中的成员函数在调用的时候才会创建（调用的时候才确定类型）
...
> push_back - 有左值和右值重载
> emplace_back - 使用万能引用（转发引用），直接传递参数给构造函数，原地构造元素。 【最大区别是emplace_back可以直接传构造参数，少一次拷贝构造】
...
是⼀种先进后出的数据结构，只有⼀个出口，stack 允许从最顶端新增元素，移除最顶端元素，取得最顶端元素。 deque 是双向开口的数据结构，所以使⽤ deque 作为底部结构并封闭其头端开口，就形成了⼀个 stack。【先进后出，push_back往里插，pop_back往里拿】

## queue

是⼀种先进先出的数据结构，有两个出口，允许从最底端加入元素，取得最顶端元素，从最底端新增元素，从最顶端移除元素。deque 是双向开⼝的数据结构，若以 deque 为底部结构并封闭其底端的出口，和头端的⼊口就形成了⼀个 queue。（其实 list 也可以实现 deque）【先进先出，push_back往里塞，pop_front从里拿】
